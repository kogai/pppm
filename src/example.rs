...
fn main() {
          ...
          if let Ok(event) = channel_receiver.try_recv() {
            match event {
              crust::Event::NewMessage(peer_id, _, bytes) => {
                ...
                println!(
                  ...
                  String::from_utf8(bytes)
                    .unwrap_or_else(|_| format!("non-UTF-8 message of {} bytes", message_length))
                );
              }
              ...
              crust::Event::BootstrapConnect(peer_id, addr) => {
                ...
                let peer_index =
                  handle_new_peer(&unwrap!(service.lock()), network2.clone(), peer_id);
              ...
              crust::Event::ConnectSuccess(peer_id) => {
                ...
                let _ = handle_new_peer(&unwrap!(service.lock()), network2.clone(), peer_id);
              }
  ...
  if running_speed_test {
    ...
    loop {
      ...
      for _ in 0..times {
        unwrap!(unwrap!(service.lock()).send(peer_id, generate_random_vec_u8(length as usize), 0));
        debug!(
        ...
        UserCommand::Send(peer_index, message) => {
          ...
          match network.get_peer_id(peer_index) {
            Some(ref mut peer_id) => {
              unwrap!(unwrap!(service.lock()).send(*peer_id, message.into_bytes(), 0));
            }
...
fn parse_user_command(cmd: &str) -> Option<UserCommand> {
  let app = App::new("cli")
    ...
    .subcommand(SubCommand::with_name("prepare-connection-info").about("Prepare a connection info"))
    .subcommand(
    ...
    .subcommand(SubCommand::with_name("list").about("List existing connections and UDP sockets"))
